...
import com.evolution.service.tesouraria.TituloService;

@RestController
@RequestMapping("/titulosFinanceiro")
public class TituloResource {
	...
	public ResponseEntity<Titulo> findOne(@PathVariable Long id) {
		Optional<Titulo> titulo = repository.findById(id);
		return titulo.isPresent() ? ResponseEntity.ok(titulo.get()) : ResponseEntity.notFound().build();
	}

	@GetMapping
	@ResponseBody
	@PreAuthorize("hasAnyAuthority('ROLE_DESENVOLVEDOR') and #oauth2.hasScope('read')")
	public List<Titulo> findAll(HttpServletRequest request) {
	...
	public ResponseEntity<Titulo> create(@Valid @RequestBody Titulo titulo, HttpServletResponse response) {
		Titulo salvo = service.create(titulo);
		return ResponseEntity.status(HttpStatus.CREATED).body(salvo);
	}

	@PutMapping("/{id}")
	@PreAuthorize("hasAnyAuthority('ROLE_DESENVOLVEDOR') and #oauth2.hasScope('write')")
	public ResponseEntity<Titulo> update(@PathVariable Long id, @Valid @RequestBody Titulo titulo) {
		Titulo salvo = service.update(id, titulo);
